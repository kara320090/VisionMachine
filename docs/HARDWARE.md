...
- [실제 치수 대조](HARDWARE_FIT_CHECK.md) / [PNG](assets/hardware-fit-check.png) / [SVG](assets/hardware-fit-check.svg): U-Grip 제외 이유, U-Rig Pro·파우치·보드·센서판의 mm 계산과 실물 통과 조건.
...
- UNO R4 Minima·DFR0265·DFR0759는 U-Rig Pro 아래 가로대에 묶은 파우치 안에 고정한다. 파우치 내부가 166×84×45 mm 이상일 때만 사용한다.
...
- SEN0308 토양수분 프로브는 한 개만 사용한다. 이동 중에는 211 mm 폭 파우치 바깥에 175 mm 프로브를 가로로 묶고, 측정할 때 분리해 흙에 삽입한다. 전자부와 커넥터는 흙·물 밖에 둔다.
...
이 휴대형 외형은 기구 설계 초안이다. 공개 치수 계산은 [HARDWARE_FIT_CHECK](HARDWARE_FIT_CHECK.md)에 기록했다. 휴대폰 케이스 포함 폭, 파우치 내부, 아래 가로대의 실제 스트랩 위치를 재고 통과한 뒤 조립한다. 핀·전압·주소는 아래 전기 연결표를 그대로 따른다.
...
3. 내부가 166×84×45 mm 이상인 파우치를 U-Rig Pro 아래 가로대의 뒤/아래에 T형 벨크로 타이 두 개로 감아 고정한다. 중앙 촬영 공간 안에 억지로 끼우지 않는다. 지퍼는 케이블 두께만큼만 열고 USB와 센서 케이블을 빼낸다.
...
6. SEN0334는 파우치 밖의 벨크로 고리에 고정해 공기가 통하게 한다. SEN0308은 마른 상태에서 파우치 바깥 전면에 가로로 타이 두 개를 사용해 보관하고 측정할 때만 풀어 화분마다 옮긴다.
...
6. 내부 실측을 통과한 파우치에 UNO·DFR0265·DFR0759를 절연판과 벨크로로 고정한다. SEN0334는 물방울과 직사광을 피하면서 공기가 통하는 바깥 고리에 두고, SEN0364와 SEN0206은 PT-3 위에서 같은 잎이나 과실을 향하게 한다.
