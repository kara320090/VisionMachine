...
검토 기준: 2026-09-20 제조사 문서와 판매 페이지. 아래는 전압·주소·단자 규격상 가능한 배선이며 실물 조립, 휴대폰 USB 전원/통신 시험, 작물 진단 유효성 검증을 대신하지 않는다.
...
- [공구 없이 조립한 시제품 예상 모습](assets/hardware-08-tool-free-prototype.png): 치수 검토 전 만든 이전 U-Grip 시각화이며 구매·장착 근거로 사용하지 않는다.
- [실제 치수 대조](HARDWARE_FIT_CHECK.md) / [PNG](assets/hardware-fit-check.png) / [SVG](assets/hardware-fit-check.svg): U-Grip 제외 이유, U-Rig Pro·EVA 하드 케이스·보드·센서판의 mm 계산과 실물 통과 조건.
- [공구 없는 조립·장착 상세 안내](HARDWARE_ASSEMBLY_GUIDE.md): 부품 확인부터 센서 배선·케이스 수납·그립 장착·측정까지 5단계 그림과 점검표.

## 사용성 개선 권장 구조

초기 분산형 배치처럼 센서 보드를 각각 케이블 끝에 매달지 않는다. 사용자가 한 장치로 같은 대상의 사진과 실측을 순서대로 얻을 수 있도록 다음 기구 구성을 권장한다.

- 휴대폰은 가장자리 클램프로 고정하고 후면 카메라를 가리지 않는다. 본체와는 짧은 USB-C 데이터 케이블 하나만 연결한다.
- UNO R4 Minima·DFR0265·DFR0759는 U-Rig Pro 아래 가로대에 묶은 EVA 하드 케이스 안에 고정한다. 케이스 내부가 166×84×45 mm 이상일 때만 사용한다.
...
- SEN0308 토양수분 프로브는 한 개만 사용한다. 이동 중에는 230 mm 폭 케이스 바깥에 175 mm 프로브를 가로로 묶고, 측정할 때 분리해 흙에 삽입한다. 전자부와 커넥터는 흙·물 밖에 둔다.
...
이 휴대형 외형은 기구 설계 초안이다. 공개 치수 계산은 [HARDWARE_FIT_CHECK](HARDWARE_FIT_CHECK.md)에 기록했다. 휴대폰 케이스 포함 폭, EVA 케이스 내부, 아래 가로대의 실제 스트랩 위치를 재고 통과한 뒤 조립한다. 핀·전압·주소는 아래 전기 연결표를 그대로 따른다.

### 공구 없이 직접 만드는 시제품 구조

위 그림의 매끈한 권총형 외장은 사용 형태를 보여 주는 개념도이며 그대로 제작하는 설계도가 아니다. 제출 기간 안에 직접 조립할 시제품은 드라이버·드릴·납땜 없이 다음 순서로 만든다.

1. 휴대폰 케이스 포함 폭이 50.8~88.9 mm인지 잰 뒤 Ulanzi U-Rig Pro의 내장 클램프에 끼운다.
2. UNO에 DFR0265를 핀 위치에 맞춰 손으로 수직 장착한다. UNO·확장판과 DFR0759는 절연판에 벨크로로 고정하고, 절연판과 EVA 하드 케이스 안쪽만 듀얼락으로 고정한다.
3. 내부가 166×84×45 mm 이상인 케이스를 U-Rig Pro 아래 가로대의 뒤/아래에 T형 벨크로 타이 두 개로 감아 고정한다. 중앙 촬영 공간 안에 억지로 끼우지 않는다. 전용 케이블 구멍이 없으므로 뚫지 않고 지퍼를 케이블 두께만큼만 열어 USB와 센서 케이블을 빼낸다.
4. U-Rig Pro 위 콜드슈에 미니 볼헤드를 끼우고 손노브로 조인다. 그 위에 PT-3 확장판을 손으로 돌려 고정한다.
5. SEN0364와 SEN0206 뒷면을 PT-3 위에 듀얼락으로 나란히 붙인다. 측정 창과 커넥터를 막지 않는다. 두 센서는 같은 잎이나 과실을 향하게 볼헤드 각도를 손으로 조정한다.
6. SEN0334는 케이스 밖의 벨크로 고리에 고정해 공기가 통하게 한다. SEN0308은 마른 상태에서 케이스 바깥 전면에 가로로 타이 두 개를 사용해 보관하고 측정할 때만 풀어 화분마다 옮긴다.
7. Gravity 완성 케이블은 편조 슬리브 한 줄로 묶는다. 슬리브 끝은 재사용 벨크로 타이로 잡아당김을 막는다.

이 구성에는 십자드라이버·드릴·리머·볼트·납땜이 필요하지 않다. 듀얼락 접착면을 닦을 마른 천과 가위만 있으면 되며, 벨크로 타이가 맞는 길이면 가위도 필요 없다. 케이스는 지퍼 틈으로 케이블을 빼므로 방수 장치로 간주하지 않는다. 센서 보드가 밖으로 노출되는 시제품이므로 물을 주는 동안에는 장치를 떼어 둔다. 구매 상한과 링크는 [PURCHASE_PLAN](PURCHASE_PLAN.md)에 정리했다.
...
6. 내부 실측을 통과한 케이스에 UNO·DFR0265·DFR0759를 절연판과 벨크로로 고정한다. SEN0334는 물방울과 직사광을 피하면서 공기가 통하는 바깥 고리에 두고, SEN0364와 SEN0206은 PT-3 위에서 같은 잎이나 과실을 향하게 한다.
...
- [U-Rig Pro 치수와 클램프 범위](https://www.bhphotovideo.com/c/product/1487529-REG/ulanzi_0673_u_rig_pro_smartphone_video.html/specs), [U-Grip Pro 구성 확인](https://www.herogear.com.au/ulanzi-u-grip-pro-camera-handle-grip-with-1-4-20-t)
