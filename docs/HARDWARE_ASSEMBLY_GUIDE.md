...
이 문서는 스마트폰 클램프가 내장된 U-Rig Pro, 외형 230×150×70 mm인 지퍼형 EVA 하드 케이스, 벨크로, 듀얼락과 손노브를 사용한 시제품 조립 순서다. 드라이버·드릴·납땜은 사용하지 않는다. 기존 생성 그림의 U-Grip 배치는 치수 검토 전 시각화이므로 구매 근거로 쓰지 않는다. **전기 연결은 반드시 모듈 인쇄와 제조사 배선도를 우선한다.**

## 1. 부품과 센서 확인

![휴대형 부품 치수 대조](assets/hardware-fit-check.png)

U-Rig Pro는 외형 폭 250.8 mm, 휴대폰 클램프 범위 50.8~88.9 mm다. EVA 하드 케이스의 외형은 230×150×70 mm이며 내부 치수는 공개되지 않았다. 제어부에 필요한 내부 공간 166×84×45 mm를 실물에서 잰 뒤 사용한다. 자세한 산식은 [HARDWARE_FIT_CHECK](HARDWARE_FIT_CHECK.md)에 있다.
...
Gravity 완성 케이블 4개, USB-C 데이터 케이블, 편조 슬리브, 벨크로 타이와 듀얼락도 확인한다. 제조사 출고 목록대로라면 DFR0759에 입력용 4핀 케이블 1개, SEN0334·SEN0364·SEN0206에 센서용 4핀 케이블이 각 1개 포함된다. 모듈 수량이나 종류가 다르면 임의로 연결하지 않는다.
...
## 3. EVA 하드 케이스에 제어보드 수납

![케이스 안 제어보드 고정 개념](assets/assembly-03-pouch.png)

1. 케이스 내부를 재서 가로 166 mm, 세로 84 mm, 깊이 45 mm 이상인지 확인한다. 하나라도 작으면 조립하지 않고 교환한다. 통과하면 안쪽의 평평한 절연 칸막이와 케이블 출구 부분을 마른 천으로 닦는다. 케이스에 칸막이가 없다면 전자부품 포장에 들어 있던 평평한 절연 플라스틱판을 사용하되 금속판은 사용하지 않는다.
...
4. 듀얼락은 PCB가 아니라 절연 칸막이 바닥과 케이스 안쪽에만 붙인다. 두 듀얼락 면을 손으로 눌러 칸막이 전체를 케이스에 고정한다.
5. SEN0334·SEN0364·SEN0206은 케이스 안에 넣지 않고 밖으로 빼 둔다. 토양수분 케이블과 USB 케이블도 같은 출구 방향으로 정리한다.
6. 이 케이스에는 전용 케이블 구멍이 없다. 구멍을 뚫지 말고 지퍼를 케이블 두께만큼 8~12 mm 정도 남겨 둔다. 케이블이 지퍼 이빨에 눌리지 않게 하고, 출구에서 2~3 cm 떨어진 곳을 벨크로 타이로 케이스 또는 그립 가로대에 묶어 당김을 막는다.

이 방식은 케이스·절연판·PCB에 구멍을 뚫지 않는다. 보드가 칸막이에서 미끄러지거나 밑면이 금속과 닿으면 전원을 넣지 않고 벨크로 위치와 절연 칸막이를 먼저 보완한다.

## 4. 휴대폰 그립과 센서 장착

![그립·케이스·센서 치수 대조](assets/hardware-fit-check.png)

1. 휴대폰 케이스 포함 폭을 재서 50.8~88.9 mm이면 U-Rig Pro 중앙 클램프에 가로로 끼운다. 후면 카메라와 USB-C 단자가 가려지지 않는지 확인한다.
2. 닫은 케이스를 중앙 촬영 공간 안에 끼우지 말고 아래 가로대의 뒤/아래에 가로로 댄다. 넓은 벨크로 타이 두 개를 서로 떨어뜨려 감고, 케이스 폭 230 mm가 그립 외형 폭 250.8 mm 안에 중앙 정렬되게 한다. 양쪽에 약 10.4 mm씩 외형 여유가 생기지만 손잡이를 잡는 공간은 실물에서 다시 확인한다.
...
7. SEN0308은 완전히 마른 상태에서 케이스 바깥 전면에 가로로 놓고 벨크로 두 개로 고정한다. 175 mm 프로브를 230 mm 케이스 중앙에 놓으면 좌우 27.5 mm씩 남는다. 금속 끝과 케이블 연결부가 양쪽 손잡이 밖으로 나오지 않게 한다.
8. 케이블은 그립 프레임을 따라 편조 슬리브로 한 줄로 정리한다. 손잡이, 카메라, 센서 감지부와 손노브를 가리지 않는다.

완성 후 장치를 양손으로 들어 좌우로 천천히 기울여 본다. 케이스·센서판·프로브가 움직이거나 케이스가 손가락에 닿으면 전원을 연결하지 않고 위치와 벨크로를 다시 조정한다.
...
4. 토양 측정이 필요하면 SEN0308을 케이스 바깥에서 풀어 줄기와 화분 벽에서 떨어진 정한 위치에 수직으로 삽입한다. 금속 프로브만 흙에 넣고 검은 전자부·커넥터는 흙과 물 밖에 둔다.
5. 앱에서 사진과 센서값이 같은 검사 ID로 묶였는지 확인한 뒤 서버로 전송한다. 실패 시 원본 사진과 측정값을 유지하고 재전송한다.
6. 측정이 끝나면 프로브의 흙을 마른 천으로 닦고 완전히 마른 상태에서 케이스 바깥 전면에 다시 고정한다.
...
- 케이스와 외부 센서는 물이 닿지 않는 상태다.
- 휴대폰 폭과 케이스 내부 치수가 [HARDWARE_FIT_CHECK](HARDWARE_FIT_CHECK.md)의 통과 기준을 만족한다.
