...
이 문서는 U-Grip, 지퍼형 전자기기 파우치, 벨크로, 듀얼락과 손노브를 사용한 시제품 조립 순서다. 드라이버·드릴·납땜은 사용하지 않는다. 그림은 조립 위치를 설명하는 시각화이므로 보드의 실제 색상·커넥터 위치·케이블 색은 구매한 제품과 다를 수 있다. **전기 연결은 반드시 모듈 인쇄와 제조사 배선도를 우선한다.**

## 1. 부품과 센서 확인

![조립 부품과 센서 전체](assets/assembly-01-parts.png)

그림의 위쪽은 Android 휴대폰, U자형 스마트폰 그립, 중형 전자기기 파우치다. 가운데는 UNO R4 Minima, DFR0265 확장판, DFR0759 I²C 허브, PT-3 확장판과 손노브 볼헤드다. 아래쪽의 네 센서는 다음과 같다.
...
1. 파우치 안쪽의 평평한 칸막이와 케이블 출구 부분을 마른 천으로 닦는다.
2. 보드 뒷면의 납땜부에 듀얼락 접착면을 직접 붙이지 않는다. 파우치의 절연 칸막이를 바닥으로 사용하고, 내장 탄성밴드나 벨크로가 UNO의 빈 모서리 부분만 잡도록 고정한다.
3. UNO·DFR0265 적층 보드와 DFR0759 사이에 손가락 한 마디 정도 여유를 두어 커넥터가 꺾이지 않게 한다.
4. SEN0334·SEN0364·SEN0206은 파우치 안에 넣지 않고 밖으로 빼 둔다. 토양수분 케이블과 USB 케이블도 같은 출구 방향으로 정리한다.
5. 지퍼는 케이블 두께만큼만 남기고 닫는다. 케이블이 지퍼 이빨에 눌리지 않게 하고, 출구 바깥에서 벨크로 타이로 묶어 당김을 막는다.

파우치의 내장 밴드가 보드를 안정적으로 잡지 못하거나 보드 밑면이 금속과 닿으면 전원을 넣지 않는다. 절연 칸막이와 고정 상태를 먼저 보완한다.

## 4. 휴대폰 그립과 센서 장착

![휴대폰 그립과 네 센서 장착](assets/assembly-04-grip-mount.png)

1. 휴대폰을 U-Grip 중앙 클램프에 가로로 끼운다. 후면 카메라와 USB-C 단자가 가려지지 않는지 확인한다.
2. 닫은 파우치를 아래 가로대 중앙에 대고 넓은 벨크로 타이 두 개로 감는다. 손잡이를 잡는 공간을 침범하지 않게 한다.
...
6. SEN0334는 오른쪽 손잡이 바깥쪽의 느슨한 벨크로 고리에 고정해 공기가 통하게 한다. 손이나 휴대폰 열이 직접 닿지 않게 한다.
7. SEN0308은 사용하지 않을 때 왼쪽 손잡이에 벨크로 두 개로 고정한다. 금속 프로브 끝이 손보다 아래로 튀어나오지 않게 한다.
8. 케이블은 그립 프레임을 따라 편조 슬리브로 한 줄로 정리한다. 손잡이, 카메라, 센서 감지부와 손노브를 가리지 않는다.

완성 후 장치를 양손으로 들어 좌우로 천천히 기울여 본다. 파우치·센서판·프로브가 움직이면 전원을 연결하기 전에 벨크로와 손노브를 다시 조인다.
...
4. 토양 측정이 필요하면 SEN0308을 손잡이에서 풀어 줄기와 화분 벽에서 떨어진 정한 위치에 수직으로 삽입한다. 금속 프로브만 흙에 넣고 검은 전자부·커넥터는 흙과 물 밖에 둔다.
5. 앱에서 사진과 센서값이 같은 검사 ID로 묶였는지 확인한 뒤 서버로 전송한다. 실패 시 원본 사진과 측정값을 유지하고 재전송한다.
6. 측정이 끝나면 프로브의 흙을 마른 천으로 닦고 완전히 마른 상태에서 손잡이에 다시 고정한다.
