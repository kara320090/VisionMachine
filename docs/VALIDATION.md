...
| 검사 | 실제 결과 |
|---|---|
| `python -m pytest -q` | **45 passed**, 외부 라이브러리 deprecation 경고 2개 |
...
- SQLite 초기 구조 재적용, 외래키 거부와 삭제 연결. 실제 서버 저장 기능을 시험한 것은 아님.
...
## 아직 검증하지 않은 것
Android SDK/Gradle 빌드·실제 APK, 카메라/USB 실휴대폰 시험, 실센서 보정·펌웨어 업로드, 사진 저장 서버, 사용자 인증·HTTPS·외부 배포, 실제 데이터 수집·학습·AI 성능, GitHub Actions 원격 실행, Play Console 등록/심사.
